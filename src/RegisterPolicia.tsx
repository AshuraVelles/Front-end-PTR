...
function RegisterPolicia() {
  ...
  const handleRegisterPolicia = async () => {
    if (!validate()) return;
    ...
    try {
      const response = await fetch('http://localhost:3995/v1/users/register', {
        method: 'POST',
        headers: {
    ...
    <div className="login-container">
      <div className={`login-box ${hasErrors ? 'error-active' : ''}`}>
        ...
        <div className="input-container">
          <div className='left'>
            <label>Escolhe um Username  POLICIA</label>
            <input
              type="text"
          ...
          <button className="register-button" onClick={handleRegisterPolicia}>Criar Conta</button>
        </div>
        </div>
...
export default RegisterPolicia;
