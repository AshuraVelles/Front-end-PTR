...
const AuctionsPage: React.FC = () => {
  ...
  useEffect(() => {
    console.log("Effect triggered with selectedAuctionId:", selectedAuctionId);
    if (selectedAuctionId !== null) {
      const selectedAuction = auctions.find(
        (auction) => auction.id === selectedAuctionId
      );
      console.log("Selected Auction:", selectedAuction);
      if (selectedAuction && !selectedAuction.ativo) {
        console.log(
          "Auction is inactive. Fetching highest bidder for auction ID:",
          selectedAuctionId
        );
        fetchHighestBidder(selectedAuctionId);
      }
    }
  }, [selectedAuctionId, auctions, authFetch]);
  ...
  const handleCheckout = async (auctionId: number, bidValue: number) => {
    ...
    try {
      ...
      const addPagamentoResponse = await axios.post(`${apiUrl}/stripe/add-pagamento`, {
      ...
      {auctions.map((auction) => (
        <div key={auction.id} className="auction-card">
          ...
          <div className="auction-details">
            ...
            {!auction.ativo && highestBidder?.utilizador_id === userInfo?.firebase_uid && highestBidder && (
              <button onClick={() => handleCheckout(auction.id, highestBidder.valor_licitacao)} disabled={isProcessing}>
